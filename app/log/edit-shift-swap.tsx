...
const debug = createScopedLogger('EditShiftSwap');

export default function EditShiftSwapScreen() {
  ...
  // Load shift swap logs on mount (only once)
  useEffect(() => {
    if (hasLoaded || !id || isLoading) {
      return;
    }

    const log = logs.find(l => l.id === id);
    if (!log || !log.isShiftSwap || !log.shiftSwapId) {
      // Only show error if we haven't loaded yet (not after an update)
      if (!hasLoaded) {
        Alert.alert('Error', 'Shift swap log not found', [
    ...
    // Find all logs in the shift swap group
    const swapLogs = logs.filter(l => l.shiftSwapId === log.shiftSwapId && l.isShiftSwap);
    
    if (swapLogs.length === 0) {
      Alert.alert('Error', 'No shift swap logs found in group', [
        { text: 'OK', onPress: () => router.back() }
      ]);
      return;
    }

    setShiftSwapId(log.shiftSwapId);

    // Identify Person A and Person B logs
    const profileInitials = (initials || profile?.employeeInitial || '').toUpperCase();
    const personALogs = swapLogs.filter(l => 
      l.initials && l.initials.toUpperCase() === profileInitials
    );
    const personBLogs = swapLogs.filter(l => 
      !(l.initials && l.initials.toUpperCase() === profileInitials)
    );

    if (personALogs.length === 0 || personBLogs.length === 0) {
      Alert.alert('Error', 'Could not identify Person A and Person B logs', [
        { text: 'OK', onPress: () => router.back() }
      ]);
      return;
    }

    // Set hasLoaded to true BEFORE setting values to prevent auto-populate from overwriting
    setHasLoaded(true);

    // Determine if same date or different dates
    const uniqueDates = new Set(swapLogs.map(l => l.date));
    const isSameDateSwap = uniqueDates.size === 1;

    if (isSameDateSwap) {
      // SAME DATE: 2 logs
      // Person A log: has actual times
      // Person B log: has actual times
      const personALog = personALogs[0];
      const personBLog = personBLogs[0];

      const personADateStr = new Date(personALog.date).toISOString().split('T')[0];
      setPersonADate(personADateStr);
      setPersonBDate(personADateStr);
      setPersonAInitials(personALog.initials || '');
      setPersonARosteredStart(personALog.rosteredStart || '');
      setPersonARosteredFinish(personALog.rosteredFinish || '');
      setPersonARosteredTimesNA(personALog.rosteredStart === 'N/A' || personALog.rosteredFinish === 'N/A');
      setPersonAActualStart(personALog.actualStart || '');
      setPersonAActualFinish(personALog.actualFinish || '');
      
      setPersonBInitials(personBLog.initials || '');
      setPersonBName(personBLog.swapPartnerName || '');
      setPersonBPayrollNumber(personBLog.swapPartnerPayrollNumber || '');
      setPersonBPayLevel(personBLog.swapPartnerPayLevel || '');
      setPersonBRosteredStart(personBLog.rosteredStart || '');
      setPersonBRosteredFinish(personBLog.rosteredFinish || '');
      setPersonBRosteredTimesNA(personBLog.rosteredStart === 'N/A' || personBLog.rosteredFinish === 'N/A');
      setPersonBActualStart(personBLog.actualStart || '');
      setPersonBActualFinish(personBLog.actualFinish || '');
      
      setMealBreakMinutes(personALog.mealBreakMinutes || 30);
    } else {
      // DIFFERENT DATES: 4 logs
      // Person A Date 1: rostered times, actual='N/A'
      // Person A Date 2: rostered='N/A', actual times
      // Person B Date 1: rostered='N/A', actual times
      // Person B Date 2: rostered times, actual='N/A'
      
      const personALogDate1 = personALogs.find(l => l.actualStart === 'N/A' && l.actualFinish === 'N/A');
      const personALogDate2 = personALogs.find(l => l.actualStart !== 'N/A' && l.actualFinish !== 'N/A');
      const personBLogDate1 = personBLogs.find(l => l.actualStart !== 'N/A' && l.actualFinish !== 'N/A');
      const personBLogDate2 = personBLogs.find(l => l.actualStart === 'N/A' && l.actualFinish === 'N/A');

      if (!personALogDate1 || !personALogDate2 || !personBLogDate1 || !personBLogDate2) {
        Alert.alert('Error', 'Could not identify all shift swap logs', [
          { text: 'OK', onPress: () => router.back() }
        ]);
        return;
      }

      const personADateStr = new Date(personALogDate1.date).toISOString().split('T')[0];
      const personBDateStr = new Date(personBLogDate2.date).toISOString().split('T')[0];
      
      setPersonADate(personADateStr);
      setPersonBDate(personBDateStr);
      setPersonAInitials(personALogDate1.initials || '');
      setPersonARosteredStart(personALogDate1.rosteredStart || '');
      setPersonARosteredFinish(personALogDate1.rosteredFinish || '');
      setPersonARosteredTimesNA(personALogDate1.rosteredStart === 'N/A' || personALogDate1.rosteredFinish === 'N/A');
      setPersonAActualStart(personALogDate2.actualStart || '');
      setPersonAActualFinish(personALogDate2.actualFinish || '');
      
      setPersonBInitials(personBLogDate1.initials || '');
      setPersonBName(personBLogDate1.swapPartnerName || '');
      setPersonBPayrollNumber(personBLogDate1.swapPartnerPayrollNumber || '');
      setPersonBPayLevel(personBLogDate1.swapPartnerPayLevel || '');
      setPersonBRosteredStart(personBLogDate2.rosteredStart || '');
      setPersonBRosteredFinish(personBLogDate2.rosteredFinish || '');
      setPersonBRosteredTimesNA(personBLogDate2.rosteredStart === 'N/A' || personBLogDate2.rosteredFinish === 'N/A');
      setPersonBActualStart(personBLogDate1.actualStart || '');
      setPersonBActualFinish(personBLogDate1.actualFinish || '');
      
      setMealBreakMinutes(personALogDate1.mealBreakMinutes || 30);
    }

    setIsLoading(false);
  }, [id, logs, router, profile, initials, hasLoaded]);

  // Auto-populate Person B's times from Person A's times
  // Simplified: User only enters Person A rostered and actual times
  // Don't auto-populate if we're still loading or haven't loaded initial data yet
  useEffect(() => {
  ...
  }, [personARosteredStart, personARosteredFinish, personAActualStart, personAActualFinish, personARosteredTimesNA, isDifferentDates, isLoading, hasLoaded]);
  ...
  const handleSave = async (status: 'draft' | 'ready') => {
    if (!shiftSwapId) {
      Alert.alert('Error', 'Shift swap ID not found');
      return;
    }
    ...
    try {
      ...
      // Show success alert after a brief delay to allow navigation
      setTimeout(() => {
        Alert.alert(
          'Success',
      ...
      debug.error('Shift swap update error:', error);
      Alert.alert('Error', `Failed to update shift swap: ${error instanceof Error ? error.message : 'Unknown error'}. Please try again.`);
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <View style={styles.content}>
            ...
            <View style={[styles.infoBanner, isDark && styles.darkInfoBanner]}>
              <Text style={[styles.infoText, isDark && styles.darkText]}>
                {isDifferentDates 
                  ? 'Edit shift swap across different dates. Enter Person A\'s rostered times (Date 1) and actual times (Date 2). Person B\'s times will auto-populate. This will update 4 log entries.'
                  : 'Edit shift swap entry. Person B\'s times will automatically populate from Person A\'s times. Enter Person A\'s shift details, then fill in Person B\'s name and details.'}
              </Text>
            </View>
